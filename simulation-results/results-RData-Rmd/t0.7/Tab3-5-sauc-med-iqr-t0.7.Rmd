...
table.sauc <- function(list.n, row.n){
  ...
  med <- 100*(apply(DATA[10, , ,drop=FALSE], 1:2, function(x) median(x, na.rm = TRUE)) - true.par)
...
save.path <- "~/Documents/GitHub/Manu1/SIM/SIM-ver2-1102/tab/"
```
...
longtab.f <- cbind.data.frame(no = c(1, rep(NA, 5),2, rep(NA, 5),3, rep(NA, 5),4, rep(NA, 5),5, rep(NA, 5),6, rep(NA, 5) ),
                              " " = rep(c("Prop $(\\hat{c}_1, \\hat{c}_2)$",
                                          "Prop $(c_1 = c_2)$",
                                          "Prop $(c_1 = 0)$",
                                          # "Proposed (correct)",
                                          # "Proposed (wrong)",
...
#sink(paste0(save.path, "tb-sauc-t0.7-c11.tex"))
...
kbl(longtab.f, 
  ...
  footnote(general = "Median with 25th empirical quartile (Q1) and 75th empirical quartile (Q3) and convergence rate (CR) are reported. 
  No. denotes the senario numbers.
   $S$ denotes the number of the population studies; True denotes the the true value of the SAUC.
   Prop $(hat c_1, hat c_2)$, Prop $(c_1 = c_2)$, and Prop $(c_1 = 1)$ denote 
   ...
   based on $N$ observed studies and$S$ population studies, respectively. 
...
#sink(paste0(save.path, "tb-sauc-t0.7-c10.tex"))
...
kbl(longtab.f, 
  ...
  No. denotes the senario numbers.
   $S$ denotes the number of the population studies; True denotes the the true value of the SAUC.
   Prop $(hat c_1, hat c_2)$, Prop $(c_1 = 1)$, and Prop $(c_1 = c_2)$ denote 
   the proposed method that estimates $(c_1, c_2)$, 
...
#sink(paste0(save.path, "tb-sauc-t0.7-c01.tex"))

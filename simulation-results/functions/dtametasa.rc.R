...
dtametasa.rc <- function(data,
  ...
  ##
    ...
    # 
    #   start7 <- c(0, 0, 0.1, 0.1, -0.1, b.init, c1)
    #
    ...
    # 
    #       start7[1:5] <- c(fit.m$coefficients, p1, p2, p.r)
    # 
    #     } 
    # 
    #   }  
  ...
  if(!inherits(opt,"try-error")) {
    ...
    if (!show.warn.message) a.opt.try <- suppressWarnings(try(uniroot(a.p, a.interval, extendInt="downX"), silent = TRUE)) else a.opt.try <- try(uniroot(a.p, a.interval, extendInt="downX"), silent = FALSE)

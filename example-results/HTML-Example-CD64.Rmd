...
funnel(tf.lndor, xlab = "lnDOR", 
       ...
       bg = c(colors, rep(1, sum(tf.lndor[["trimfill"]]))),
...
funnel(tf.se, xlab = "logit(sensitivity)",
       ...
       bg = c(colors, rep(1, sum(tf.se[["trimfill"]]))),
       cex = 1.5, studlab = FALSE, backtransf = FALSE
)
...
funnel(ml.sp, xlab = "logit(specificity)", fixed = FALSE,
       ...
       bg = c(colors, rep(1, sum(tf.sp[["trimfill"]]))),
       cex = 1.5, studlab = FALSE, backtransf = FALSE
)
...
legend('bottom',
       legend = c("Unpublished studies", "Published studies with cutoff values of 1800-10000 (mol/cell)",
       ...
       pt.bg = c("white", "red", "tan4", "skyblue4", "grey", "white")[legend_order],
       xpd = TRUE, 
       horiz = FALSE, 
       cex = 1.2,
